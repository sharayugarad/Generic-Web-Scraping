"""
Rankiteo scraper - Scrapes blog post URLs from blog.rankiteo.com.
"""
import logging
...
class RankiteoScraper(BaseScraper):
    """Scraper for blog.rankiteo.com blog posts."""
    
    def __init__(self, logger: logging.Logger, timeout: int = 30, max_retries: int = 3, retry_delay: int = 2):
        """
        Initialize Rankiteo scraper.
        ...
        self.source_name = "Rankiteo Blog"
        self.blog_url = "https://blog.rankiteo.com"
    
    def _try_sitemaps(self) -> List[str]:
        """
        Try to find and scrape blog sitemaps.
        ...
        for path in sitemap_paths:
            sitemap_url = self.blog_url.rstrip('/') + path
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if sitemaps:
                        ...
                        return self.parse_xml_sitemap(sitemap_url)
    ...
    def _scrape_homepage(self) -> List[str]:
        """
        Scrape URLs from the blog homepage.
        ...
        try:
            self.logger.info(f"Scraping homepage: {self.blog_url}")
            response = self._fetch_with_retry(self.blog_url)
            ...
            for link in soup.find_all('a', href=True):
                ...
                if href.startswith('/'):
                    href = self.blog_url.rstrip('/') + href
                elif not href.startswith('http'):
                    continue
                
                # Only include URLs from the same domain
                if href.startswith(self.blog_url):
                    urls.append(href)
    ...
    def scrape(self) -> List[str]:
        """
        Scrape URLs from Rankiteo blog.
